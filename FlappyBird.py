...
class FlappyBird:
    ...
    def ifCollidebottom(self,x_l_bird,x_r_bird,y_b_bird,y_t_bird,x_l_Bbar,x_r_Bbar,y_t_Bbar):
        ...
        if (x_l_Bbar <= x_r_bird and x_r_Bbar >= x_l_bird) and y_b_bird <= y_t_Bbar:
            return True
...
##    quitClicked = False
    ...
    while (not game.touchGround(y_b_bird) and not game.ifCollidebottom(x_l_bird,x_r_bird,y_b_bird,y_t_bird,x_l_Bbar,x_r_Bbar,y_t_Bbar) and not game.ifCollidetop(x_l_bird,x_r_bird,y_b_bird,y_t_bird,x_l_Bbar,x_r_Bbar,y_t_Bbar + 40)) :
        ...
        while x_r_Bbar > 0:
            ...
            if game.touchGround(y_b_bird) or game.ifCollidebottom(x_l_bird,x_r_bird,y_b_bird,y_t_bird,x_l_Bbar,x_r_Bbar,y_t_Bbar) or game.ifCollidetop(x_l_bird,x_r_bird,y_b_bird,y_t_bird,x_l_Bbar,x_r_Bbar,y_t_Bbar + 40):
                died = True
                break
...
##    quitClicked = True
    ...
    if (58 <=pt.getX() <=72 and 46<=pt.getY()<=54):
        win.close()
    #The game restarts when the Restart button is clicked
    elif (25<=pt.getX()<=45 and 46<= pt.getY()<=54):
        win.close()
        main()
